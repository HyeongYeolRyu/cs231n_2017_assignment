...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            # fc layer
            out, fc_cache = affine_forward(x, self.params['W'+str(i+1)], self.params['b'+str(i+1)])
            ...
            x = out # next layer's input
                
        out, fc_cache = affine_forward(x, self.params['W'+str(self.num_layers)], self.params['b'+str(self.num_layers)])
        fc_caches.append(fc_cache)
        scores = out
        ############################################################################
        #                             END OF YOUR CODE                             #
        ...
        ############################################################################
        loss, dscores = softmax_loss(scores, y)   
        RW = np.sum( [np.sum(self.params['W'+str(i+1)]**2) for i in range(self.num_layers)] )
        loss += 0.5 * self.reg * RW 
      
        # backward
        dx, dw, db = affine_backward(dscores, fc_caches[self.num_layers - 1])
        ...
        for i in range(self.num_layers - 1, 0, -1):
            if self.use_dropout:
                dx = dropout_backward(dx, do_caches[i - 1])
            dx = relu_backward(dx, relu_caches[i - 1])
            if self.use_batchnorm:
                dx, dgamma, dbeta = batchnorm_backward_alt(dx, bn_caches[i - 1])
                grads['gamma'+str(i)] = dgamma
                grads['beta'+str(i)] = dbeta
                
            dx, dw, db = affine_backward(dx, fc_caches[i - 1])
